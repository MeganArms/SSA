function [icounts,dtime,firstframe,lastframe,brightness,avgb,rangeb,maxb,rcdfb] = factors(spotEvents1,spotEvents2,Nframes,objs_link1,objs_link2)
...
p = 1; 
% l = 1;
firstframe = zeros(length(spotEvents1)+length(spotEvents2),1);
lastframe = firstframe;
avgb = firstframe;
rangeb = firstframe;
maxb = 0;
icounts = zeros(Nframes*(length(spotEvents1)+length(spotEvents2)),4);
dtime = zeros(Nframes*(length(spotEvents1)+length(spotEvents2)),1);
brightness = cell(length(spotEvents1)+length(spotEvents2),1);
bmat = NaN(length(spotEvents1)+length(spotEvents2),Nframes);
...
for k = 1:length(spotEvents2)
    ...
    t = idx(k);
    inds = find(~isnan(traj));
    frames = objs_link2(5,traj(inds));
    int_counts = objs_link2(3,traj(inds));
    ...
    if firstframe(t) ~= 1 && lastframe(t) ~= Nframes && n > 1
        % If the bright events do not overlap the start or end of the movie
%         btime(l:l+length(bCC.PixelIdxList)-1) = cellfun(@length,bCC.PixelIdxList);
        lengths = cellfun(@length,dCC.PixelIdxList);
        lengths = lengths(lengths>1);
...
%         l = l + length(bCC.PixelIdxList);
        ...
        for q = 1:length(icounts_hold)
            % Total integrated counts of the bright event. Error will be sqrt.
            icounts(s,1) = sum(objs_link2(3,icounts_hold{q}));
            % Length of bright event
            icounts(s,2) = length(icounts_hold{q});
            s = s + 1;
        end
        ...
        % Length of this entire connected traj in column 4
        icounts(s-q:s-1,4) = n*ones(q,1);
%     elseif firstframe(t) ~= 1 && lastframe(t) == Nframes && n > 1
%         % If the last bright event appears on the last frame
% %         blengths = cellfun(@length,bCC.PixelIdxList);
% %         btime(l:l+length(blengths)-2) = blengths(1:end-1);
%         dlengths = cellfun(@length,dCC.PixelIdxList);
