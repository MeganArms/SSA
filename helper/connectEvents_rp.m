...
if numTrajectories < 2
    ...
    for i = 1:numTrajectories
        ...
        if issorted(Result(i).trajectory) && length(Result(i).trajectory) >= 3
            firstMolecsCoords(i,:) = C{i}(1:2,1)';
%         else
%             disp([num2str(i),' is not strictly increasing'])
    ...
    % sorting
    distFromOriginCoords = sqrt(sum(firstMolecsCoords(firstMolecsCoords(:,1) ~= 0,:).^2,2));
    indices = 1:length(firstMolecsCoords);
    thIndices = indices(firstMolecsCoords(:,1) ~= 0);
    
    % Get the sorted indices
    [~, sortedIndices] = sort(distFromOriginCoords, 1, 'ascend');
    sortedTrajIndices = thIndices(sortedIndices); % Why don't these remain sorted?
    
    % Sort the coordinates of the first molecules in each trajectory according
    % to their coordinates
    sortedFirstMolecs = firstMolecsCoords(sortedTrajIndices,:);
    ...
    for i = 2:length(diffsMatrix)
        ...
        if EucDistMatrix(i) <= sqrt(th)
            if k == 1 % Then this is the first entry in the cell. Advance j by one to create a new cell.
                ...
                closeTrajs{1,j}(1,k+1) = sortedTrajIndices(i);
                ...
                if store == 1 % If the molecule is close enough to all the other molecules, store it in the trajectory!
                    closeTrajs{1,j}(1,k) = sortedTrajIndices(i);
        ...
        % elseif EucDistMatrix(i) <= sqrt(2) && abs(i - prev_i) > 1 % Create a new cell if the current index is too far from the prev
            ...
            closeTrajs{1,j}(1,k) = sortedTrajIndices(i-1);
            closeTrajs{1,j}(1,k+1) = sortedTrajIndices(i);
    ...
    if length(closeTrajs) == 1 && isempty(closeTrajs{1})
        spotEvents = obj.Result;
        return
    else
        ...
        while i <= length(traj2connect_withUpdates)
            ...
            for j = 2:n
                ...
                prevFrame = F{traj2connect_withUpdates{i}(j-1)}(end);
                currentFrame = F{traj2connect_withUpdates{i}(j)}(1);
%                 if (currentFrame - prevFrame)*exp_time <= 1 % second
                    ...
                    emptyTrajNums = [emptyTrajNums; traj2connect_withUpdates{i}(j)];
...
%                 end
            ...
            spotEvents(traj2connect_withUpdates{i}(1)).trajectory = trajComplete{i};
        ...
        if ~isempty(emptyTrajNums)
            ...
            for i = 1:length(sortedEmptyTrajNums)
                j = sortedEmptyTrajNums(i); % j is trajectory number
                if j == jPrevious
                    continue
                else
                    spotEvents(j) = [];
        ...
        for i = 1:length(spotEvents)
            [m, ~] = size(spotEvents(i).trajectory);
            if m ~= 1
                spotEvents(i).trajectory = spotEvents(i).trajectory';
            end
            notNAN = ~isnan(spotEvents(i).trajectory); % Find the real values
            % If the real values are not sorted, then go through doubles-deleting
            if ~issorted(spotEvents(i).trajectory(notNAN))
                hold = [0, spotEvents(i).trajectory];
                ...
                spotEvents(i).trajectory = spotEvents(i).trajectory(indices);
